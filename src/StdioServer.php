...
class StdioServer
{
    ...
    public function __construct(string $name, ?string $local_timezone = null)
    {
        ...
        $this->local_tz = TimeUtils::getLocalTz($local_timezone)->getName();
        $this->register_tools();
    }
    ...
    public function call_tool(string $name, array $arguments): array
    {
        try {
            ...
            return [
                [
                    'type' => 'text',
                    'text' => json_encode($result->toArray(), JSON_PRETTY_PRINT)
                ]
            ];
        } catch (Exception $e) {
     ...
     * エラーレスポンスを送信
     */
    protected function send_error_response($id, int $code, string $message): void
    {
        $response = [
            'jsonrpc' => '2.0',
            'error' => [
     ...
     * 成功レスポンスを送信
     */
    protected function send_response($id, $result): void
    {
        $response = [
            'jsonrpc' => '2.0',
            'result' => $result,
